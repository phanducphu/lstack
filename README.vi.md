...
AVN-Stack là ứng dụng desktop quản lý toàn bộ stack phát triển PHP cục bộ — web server, cơ sở dữ liệu, cache và mail — không cần Docker hay WSL. Các dịch vụ chạy dưới dạng tiến trình native, dự án được phục vụ qua domain `.test` với SSL tuỳ chọn bằng mkcert.
...
Khi tạo dự án mới, AVN-Stack có thể khởi tạo từ một trong các template sau:
...
| Domain             | Mục đích                                         |
|--------------------|--------------------------------------------------|
| `localhost.test`   | Dashboard / trang welcome của AVN-Stack             |
...
Tải bản phát hành mới nhất tại [trang Releases](https://github.com/marixdev/avnstack/releases) hoặc [avnstack.dev](https://avnstack.dev):
...
**Windows** — chạy installer NSIS và mở AVN-Stack từ Start Menu.
...
AVN-Stack lưu toàn bộ cấu hình, dữ liệu dịch vụ, chứng chỉ và binary đã tải về tại:
...
Tất cả dữ liệu đều độc lập — gỡ cài đặt AVN-Stack không ảnh hưởng đến cơ sở dữ liệu hoặc cấu hình trừ khi bạn xoá thư mục này thủ công.
...
| Cài đặt                | Mô tả                                                         |
|------------------------|---------------------------------------------------------------|
| Thư mục dự án          | Thư mục gốc chứa dự án (mặc định: `~/AVN-Stack/www/`)           |
